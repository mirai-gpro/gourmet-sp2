...
def search_google_restaurants(keyword: str, area: str = '', geo_info: dict = None,
                              language: str = 'ja', count: int = 5) -> list:
    """
    Google Places Text Search API でレストランをキーワード検索し、
    実在する営業中の店舗のみを返す。

    LiveAPI の search_restaurants ツールから直接呼び出す用途。
    仕様: Google Places で実在確認（ハルシネーション・廃業対策）→ enrich で補助リンク追加

    Returns:
        [{'name': '...', 'area': '...'}, ...] — enrich_shops_with_photos() に渡す形式
    """
    if not GOOGLE_PLACES_API_KEY:
        logger.warning("[Places Search] APIキーが設定されていません")
        return []

    # 検索クエリ構築
    region = geo_info.get('region', '') if geo_info else ''
    query = f"{keyword} {area} {region}".strip()
    ...
    try:
        ...
        logger.info(f"[Places Search] 検索: query='{query}', count={count}")
        response = requests.get(search_url, params=params, timeout=10)
        data = response.json()

        if data.get('status') != 'OK' or not data.get('results'):
            logger.info(f"[Places Search] 結果なし: {query} (status={data.get('status')})")
            return []

        # 営業中 (OPERATIONAL) の店舗のみフィルタ
        shops = []
        for place in data['results']:
            ...
            if status != 'OPERATIONAL':
                logger.info(f"[Places Search] スキップ ({status}): {name}")
                continue

            shops.append({'name': name, 'area': area})

            if len(shops) >= count:
                break

        logger.info(f"[Places Search] {len(shops)}件取得: {[s['name'] for s in shops]}")
        return shops

    except Exception as e:
        logger.error(f"[Places Search] エラー: {e}")
        return []
