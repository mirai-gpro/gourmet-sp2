...
| # | 依頼 | 根本原因（推定） | 必要な対応 |
|---|------|-----------------|-----------|
| **1** | Gemini Live API 接続不安定 | **`settings.py` のモデル名 `gemini-2.5-flash-native-audio-preview-12-2025` が期限切れ**（2025年12月プレビュー版）。`handle_client_ws()` の reconnect ループに上限がなく無限リトライ | **モデル名を最新版に更新** + ループに上限追加 |
| **2** | WebSocket 1006 切断 | 依頼1 の無限リトライの結果、Cloud Run タイムアウトで強制切断 | 依頼1 の修正で解消見込み |
| **3** | REST chat 内部エラー | 未特定（前回から継続） | Cloud Run ログでスタックトレースを取得 → 根本原因特定 |

**最優先アクション: `settings.py` の `LIVE_API_MODEL` を最新モデル名に変更してデプロイ。**
これだけで依頼1・2が解消する可能性が高い。
